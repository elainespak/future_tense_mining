    ...
    for Sentence in report.Sentences_info:
        if Sentence.aspect_label == 1:   # if the sentence has an aspect label,
            ...
            for l in Sentence.aspect_label:
                report.num_stn_aspect[0] += 1
            for w,v in Sentence.word_frequency.items():#keys():
                z = np.where(w == report.UniWord)[0]  # index
                report.num_stn_word[z] += v
            for l in Sentence.aspect_label:
                for w,v in Sentence.word_frequency.items():#keys():
                    z = np.where(w == report.UniWord)[0] # index
                    report.num_stn_aspect_word[0,z] += v # FIX HERE???
...
class Report:
    def __init__(self, ind_data, VocabDict):
        '''
        # INPUT
        ...
        self.Sentences_info = [Sentence_info(sent_in_nums) for sent_in_nums in self.Sentences_in_nums]
        UniWord = {}
...
class Report_Collection:
    def __init__(self, dat):
        self.Reports = [Report(dat['Nouns'][idx], vocab_dict) for idx in range(len(dat))]
...
all_reports = Report_Collection(dat).Reports

for report in all_reports:
    for idx in range(len(report.Sentences_in_nums)):
        match_count = sent_aspect_match(report.Sentences_in_nums[idx], aspect_terms)
        if np.max(match_count)>0: # if at least one of the aspects has a match
            report.Sentences_info[idx].aspect_label = np.ones(1)
        else:
            report.Sentences_info[idx].aspect_label = np.zeros(1)
